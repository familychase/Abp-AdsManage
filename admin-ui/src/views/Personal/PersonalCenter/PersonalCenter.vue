...
import { getSelfMenuApi } from '@/api/login'
import type { SelfInfoResponse } from '@/api/login'
...
const fetchDetailUserApi = async () => {
  try {
    ...
    const data: SelfInfoResponse = res.data
    userInfo.value = {
      userId: data.userInfo.userId,
      username: data.userInfo.userCode || '',
      realName: data.userInfo.aliasName || data.userInfo.userName || '',
    ...
    <ContentWrap title="基本资料" class="flex-[3] ml-20px">
      <ElTabs v-model="activeName">
        <ElTabPane label="基本信息" name="first">
          <EditInfo :user-info="userInfo" @saved="fetchDetailUserApi" />
        </ElTabPane>
        <ElTabPane label="修改密码" name="second">
          <EditPassword :user-id="userInfo?.userId" />
        </ElTabPane>
      </ElTabs>
    </ContentWrap>
  </div>
